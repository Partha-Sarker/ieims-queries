...
BEGIN
    ...
    DECLARE @Id int;
    DECLARE @sqlMaxId NVARCHAR(MAX) = N'SELECT @Id = MAX(id) + 100 FROM ' + @SchemaName + '.' + @CurrentTable;
    EXEC sp_executesql @sqlMaxId, N'@ID INT OUTPUT', @ID = @Id OUTPUT;

    IF @MaxId IS NOT NULL
        PRINT @Id
    BEGIN
        DECLARE @sql NVARCHAR(MAX) = N'ALTER SEQUENCE ' + @SchemaName + '.' + @CurrentTable + '_seq RESTART WITH ' + CAST(@Id + 1 AS NVARCHAR(20));
